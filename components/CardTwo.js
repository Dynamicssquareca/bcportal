            ...
            <div className="row">
                <div className="col-lg-10">
                    {loading ? (
                        ...
                        <div className="card-g-flex card-g-flex-white" style={{ display: "flex", gap: 20 }}>
                        ...
                        featured ? (
                            <div className="card-g-flex card-g-flex-white">
                                ...
                                <div className="content-one">
                                    ...
                                    <p className="card-text-w">{getExcerpt(featured, 200)}</p>
                                    <p className="text-muteds mb-2">
                                        {getAuthor(featured) ? (
                                            getAuthorHref(featured) ? (
                                                <Link href={getAuthorHref(featured)} className="link-st">
                                                    <small className="me-2 link-st-s-w">{getAuthor(featured)}</small>
                                                </Link>
                                            ) : (
                                                <small className="me-2 link-st-s-w">{getAuthor(featured)}</small>
                                            )
                                        ) : (
                                            <small className="me-2 link-st-s-w">Unknown author</small>
                                        )}
                                        {getDate(featured) && <small className="me-2 link-st-s-w">/ {getDate(featured)}</small>}
                                    </p>
                ...
                {loading ? (
                    ...
                    <>
                        {[0, 1, 2].map((i) => (
                            <div key={i} className="col-lg-3">
                                <div className="card-single-list card-single-list-w" style={{ paddingBottom: 10 }}>
                    ...
                    !error && others.map((b, idx) => (
                        <div key={b?._id ?? b?.slug ?? idx} className="col-lg-4">
                            <div className="card-single-list card-single-list-w">
                                <h3 className="card-title">
                                    <Link href={getHref(b)}>
                                        {getTitle(b)}
                                    </Link>
                                ...
                                <div className="mt-auto">
                                    <small className="text-muteda-w">
                                        {getAuthor(b) ? (
                                            getAuthorHref(b) ? (
                                                <Link href={getAuthorHref(b)} className="link-st">
                                                    <span className="me-2 link-st-s-w">{getAuthor(b)}</span>
                                                </Link>
                                            ) : (
                                                <span className="me-2 link-st-s-w">{getAuthor(b)}</span>
                                            )
                                        ) : (
                                            <span className="me-2 link-st-s-w">Unknown author</span>
                                        )}
